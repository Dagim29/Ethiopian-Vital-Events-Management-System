...
const ViewMarriageRecord = ({ isOpen = false, onClose = () => {}, record = {}, onStatusChange = () => {} }) => {
  const { user } = useAuth();
  ...
  const renderPhotos = () => {
    const photos = [
      { label: 'Groom Photo', src: record.groom_photo, icon: '🤵' },
      { label: 'Bride Photo', src: record.bride_photo, icon: '👰' }
  ...
  const getSections = () => {
    ...
    const sections = {
      ...
      'Marriage Details': [
        'marriage_date', 'marriage_place', 'marriage_type'
      ],
      ...
      return (
        <div key={title} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden mb-6">
          <div className="px-4 py-3 bg-pink-50 border-b border-pink-200">
            <h3 className="text-base font-medium text-pink-800">{title}</h3>
          </div>
  ...
  console.log('Marriage Record Data:', record);
                      ...
                      <>
                        <button
                          ...
                          onClick={async () => {
                            setIsApproving(true);
                            try {
                              await marriageRecordsAPI.approveRecord(record.marriage_id || record.id);
                              toast.success('Marriage record approved successfully');
                              onStatusChange();
                              onClose();
                            } catch (error) {
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            if (reason !== null) {
                              setIsRejecting(true);
                              try {
                                await marriageRecordsAPI.rejectRecord(record.marriage_id || record.id, reason);
                                toast.success('Marriage record rejected');
                                onStatusChange();
                                onClose();
                              } catch (error) {
...
ViewMarriageRecord.propTypes = {
  isOpen: PropTypes.bool,
  onClose: PropTypes.func,
  record: PropTypes.object
};

export default ViewMarriageRecord;
