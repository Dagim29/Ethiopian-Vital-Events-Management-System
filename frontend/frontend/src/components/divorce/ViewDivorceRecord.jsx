...
const ViewDivorceRecord = ({ isOpen = false, onClose = () => {}, record = {}, onStatusChange = () => {} }) => {
  const { user } = useAuth();
  ...
  const renderPhotos = () => {
    const photos = [
      { label: 'Husband Photo', src: record.husband_photo, icon: '👨' },
      { label: 'Wife Photo', src: record.wife_photo, icon: '👩' }
  ...
  const getSections = () => {
    ...
    const sections = {
      'Spouse 1 Information': [
        'spouse1_full_name', 'spouse1_gender', 'spouse1_date_of_birth',
        'spouse1_nationality', 'spouse1_ethnicity', 'spouse1_religion',
        'spouse1_occupation', 'spouse1_id_number', 'spouse1_phone'
      ],
      'Spouse 2 Information': [
        'spouse2_full_name', 'spouse2_gender', 'spouse2_date_of_birth',
        'spouse2_nationality', 'spouse2_ethnicity', 'spouse2_religion',
        'spouse2_occupation', 'spouse2_id_number', 'spouse2_phone'
      ],
      'Marriage and Divorce Details': [
        'marriage_date', 'divorce_date', 'divorce_reason', 'court_name', 'case_number'
      ],
      ...
      return (
        <div key={title} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden mb-6">
          <div className="px-4 py-3 bg-orange-50 border-b border-orange-200">
            <h3 className="text-base font-medium text-orange-800">{title}</h3>
          </div>
  ...
  console.log('Divorce Record Data:', record);
            ...
            >
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-5xl sm:p-6">
                ...
                <div className="mt-6 flex justify-between items-center gap-3 border-t pt-4">
                      ...
                      <>
                        <button
                          ...
                          onClick={async () => {
                            setIsApproving(true);
                            try {
                              await divorceRecordsAPI.approveRecord(record.divorce_id || record.id);
                              toast.success('Divorce record approved successfully');
                              onStatusChange();
                              onClose();
                            } catch (error) {
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            if (reason !== null) {
                              setIsRejecting(true);
                              try {
                                await divorceRecordsAPI.rejectRecord(record.divorce_id || record.id, reason);
                                toast.success('Divorce record rejected');
                                onStatusChange();
                                onClose();
                              } catch (error) {
                  ...
                  <button
                    type="button"
                    className="rounded-md bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 border border-gray-300"
                    onClick={onClose}
                  >
                    Close
                  </button>
...
ViewDivorceRecord.propTypes = {
  isOpen: PropTypes.bool,
  onClose: PropTypes.func,
  record: PropTypes.object
};

export default ViewDivorceRecord;
